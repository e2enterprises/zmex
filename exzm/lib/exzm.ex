defmodule Exzm.EncrustedNif do
  ...
  def process_zmachine_input(_story_data, _state_data, _input_strings) do
...
defmodule Exzm do
  ...
  def new_game(story_data, [first_input | rest_inputs], opts)
    ...
    if Keyword.get(opts, :diagnostics) do
      process_zmachine_inputs_with_diagnostics(story_data, <<>>, inputs)
    else
      process_zmachine_inputs(story_data, <<>>, inputs)
    end
  end
  ...
  def continue(story_data, save_data, [first_input | rest_inputs], opts)
    ...
    if Keyword.get(opts, :diagnostics) do
      process_zmachine_inputs_with_diagnostics(story_data, save_data, inputs)
    else
      process_zmachine_inputs(story_data, save_data, inputs)
    end
  end
  ...
  defp process_zmachine_inputs(story_data, save_data, inputs) do
    ...
    {new_save_data, first_output} =
      EncrustedNif.process_zmachine_input(story_data, save_data, first_input)
  ...
  defp process_zmachine_input_with_diagnostics(story_data, save_data, input) do
    {nif_duration, {new_save_data, output}} =
      :timer.tc(
        &EncrustedNif.process_zmachine_input/3,
  ...
  defp process_zmachine_inputs_with_diagnostics(story_data, save_data, inputs) do
    [first_input | rest_inputs] = inputs
